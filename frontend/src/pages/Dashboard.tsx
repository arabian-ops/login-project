...
const AdvancedDashboard = () => {
  ...
  const addTask = useCallback(async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await api.post("/api/tasks", { title: newTask }, { headers: { Authorization: `Bearer ${token}` } });
      if (res.data.success) {
  ...
  const completeTask = async (id: number) => {
    try {
      const token = localStorage.getItem("token");
      const res = await api.patch(`/api/tasks/${id}/toggle`, {}, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const saveEdit = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await api.patch(`/api/tasks/${editingId}`, { title: editText }, { headers: { Authorization: `Bearer ${token}` } });
      if (res.data.success) {
